...
export class Pool extends TokenContract {
  ...
  @method async createPair(_token0: PublicKey, _token1: PublicKey) {
  ...
  async mintLiquidity(_amount0: UInt64, _amount1: UInt64) {
    ...
    if (_poolState.totalSupply.equals(new UInt64(0))) {
      // use field for sqrt
      ...
      liquidity = _amount0.mul(_poolState.totalSupply).div(_poolState.reserve0)
    }
